...
public class PptExtractorTest extends AbstractDocumentExtractorTest {
  ...
  protected File getTestFile() {
    URL resource = PptExtractorTest.class.getResource("testPresentation.ppt");
    try {
  ...
  protected void validateText(Collection<Text> textContents) {
    ...
    assertEquals(
        "Slide1", note.getProperties().get(PropertyKeys.PROPERTY_KEY_NAME).orElse("UNKNOWN"));
    ...
    assertEquals(
        "Slide2", comment.getProperties().get(PropertyKeys.PROPERTY_KEY_NAME).orElse("UNKNOWN"));
  }
